<?php

namespace admin\controllers;

use admin\controllers\AdminController;
use admin\modules\rbac\components\RbacHtml;
use admin\widgets\sortableGridView\SortableAction;
use common\components\helpers\UserUrl;
use common\models\Answers;
use common\models\AnswersSearch;
use common\models\Questions;
use common\models\QuestionsSearch;
use himiklab\sortablegrid\SortableGridAction;
use kartik\grid\EditableColumnAction;
use Throwable;
use Yii;
use yii\base\InvalidConfigException;
use yii\db\StaleObjectException;
use yii\filters\VerbFilter;
use yii\helpers\ArrayHelper;
use yii\web\NotFoundHttpException;
use yii\web\Response;
use Yii2\Extensions\DynamicForm\Models\Model;

/**
 * QuestionsController implements the CRUD actions for Questions model.
 *
 * @package admin\controllers
 */
final class QuestionsController extends AdminController
{
    /**
     * {@inheritdoc}
     */
    public function behaviors(): array
    {
        return ArrayHelper::merge(parent::behaviors(), [
            'verbs' => [
                'class' => VerbFilter::class,
                'actions' => ['delete' => ['POST']]
            ]
        ]);
    }

    /**
     * Lists all Questions models.
     *
     * @throws InvalidConfigException
     */
    public function actionIndex(): string
    {
        $model = new Questions();

        if (RbacHtml::isAvailable(['create']) && $model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', "Элемент №$model->id создан успешно");
        }

        $searchModel = new QuestionsSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);

        return $this->render(
            'index',
            ['searchModel' => $searchModel, 'dataProvider' => $dataProvider, 'model' => $model]
        );
    }

    /**
     * Displays a single Questions model.
     *
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionView(int $id): string
    {
        return $this->render('view', ['model' => $this->findModel($id)]);
    }

    /**
     * Creates a new Questions model.
     *
     * If creation is successful, the browser will be redirected to the 'view' page.
     *
     * @param string|null $redirect если нужен иной редирект после успешного создания
     *
     * @throws InvalidConfigException
     */
    public function actionCreate(string $redirect = null): Response|string
    {
        $model = new Questions();
        $answers = [new Answers()];

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $answers = Model::createMultiple(Answers::class);
            Model::loadMultiple($answers, Yii::$app->request->post(), 'Answers');

            if (Model::validateMultiple($answers) && $model->save()) {
                foreach ($answers as $answer) {
                    $answer->question_id = $model->id;
                    $answer->save();
                }
                Yii::$app->session->setFlash('success', "Элемент №$model->id создан успешно");
                return match ($redirect) {
                    'create' => $this->redirect(['create']),
                    'index' => $this->redirect(UserUrl::setFilters(QuestionsSearch::class)),
                    default => $this->redirect(['view', 'id' => $model->id])
                };
            }
        }

        return $this->render('create', [
            'model' => $model,
            'answers' => (empty($answers)) ? [new Answers()] : $answers,
        ]);
    }

    /**
     * Updates an existing Questions model.
     *
     * If update is successful, the browser will be redirected to the 'view' page.
     *
     * @throws NotFoundHttpException if the model cannot be found
     * @throws InvalidConfigException
     */
    public function actionUpdate(int $id): Response|string
    {
        $model = $this->findModel($id);
        $answers = $model->answers;

        $pkey = 'id';

        if (empty($answers)) {
            $answers = [new Answers()];
        }

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $oldIDs = ArrayHelper::map($answers, $pkey, $pkey);
            $answers = Model::createMultiple(Answers::class, $answers, $pkey);
            Model::loadMultiple($answers, $this->request->post(), 'Answers');
            $deletedIDs = array_diff($oldIDs, array_filter(ArrayHelper::map($answers, $pkey, $pkey)));

            if (!empty($deletedIDs)) {
                Answers::deleteAll([$pkey => $deletedIDs]);
            }

            $valid = $model->validate();
            $valid = Model::validateMultiple($answers) && $valid;

            if ($valid) {
                if ($model->save()) {
                    foreach ($answers as $answer) {
                        $answer->question_id = $model->id;
                        $answer->save();
                    }
                    Yii::$app->session->setFlash('success', "Элемент №$model->id изменен успешно");
                    return $this->redirect(['view', 'id' => $model->id]);
                }
            }
        }

        return $this->render('update', [
            'model' => $model,
            'answers' => (empty($answers)) ? [new Answers()] : $answers,
        ]);
    }

    /**
     * Deletes an existing Questions model.
     *
     * If deletion is successful, the browser will be redirected to the 'index' page.
     *
     * @throws NotFoundHttpException if the model cannot be found
     * @throws StaleObjectException
     * @throws Throwable
     */
    public function actionDelete(int $id): Response
    {
        $this->findModel($id)->delete();
        Yii::$app->session->setFlash('success', "Элемент №$id удален успешно");
        return $this->redirect(UserUrl::setFilters(QuestionsSearch::class));
    }

    /**
     * Finds the Questions model based on its primary key value.
     *
     * If the model is not found, a 404 HTTP exception will be thrown.
     *
     * @throws NotFoundHttpException if the model cannot be found
     */
    private function findModel(int $id): Questions
    {
        if (($model = Questions::findOne($id)) !== null) {
            return $model;
        }

        throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
    }

    /**
     * {@inheritdoc}
     */
    public function actions(): array
    {
        return [
            'change' => [
                'class' => EditableColumnAction::class,
                'modelClass' => Questions::class
            ],
            'sort' => [
                'class' => SortableAction::class,
                'modelClass' => Questions::class,
                'orderColumn' => 'position'
            ]
        ];
    }
}
